...
int MarchingCubes<T>::getCellCode(int32 const cx, int32 const cy, int32 const cz, VolumeDataType const iso) const {
...
template<class T> inline
int MarchingCubes<T>::getDualPointCode(int32 const cx, int32 const cy, int32 const cz, VolumeDataType const iso, DMCEdgeCode const edge) const {
    ...
    if (generateManifold) {
        ...
        uint8 const direction = problematicConfigs[uint8(cubeCode)];
        ...
        if (direction != 255) {
            ...
            if (neighborCoords[component] >= 0 && neighborCoords[component] < (dims[component] - 1)) {
                ...
                if (problematicConfigs[uint8(neighborCubeCode)] != 255) {
...
template<class T> inline
void MarchingCubes<T>::calculateDualPoint(int32 const cx, int32 const cy, int32 const cz, VolumeDataType const iso, int const pointCode, Vector3f& v) const {
...
template<class T> inline
QuadIndexType MarchingCubes<T>::getSharedDualPointIndex(
    int32 const cx, int32 const cy, int32 const cz,
...
template<class T> inline
void MarchingCubes<T>::buildQuadSoup(
    VolumeDataType const iso,
    std::vector<Vector3f>& vertices,
    std::vector<Quad>& quads
) {

    int32 const reducedX = dims[0] - 2;
    int32 const reducedY = dims[1] - 2;
    int32 const reducedZ = dims[2] - 2;
    ...
    // iterate voxels
    for (int32 z = 0; z < reducedZ; ++z)
        for (int32 y = 0; y < reducedY; ++y)
            for (int32 x = 0; x < reducedX; ++x) {
...
template<class T> inline
void MarchingCubes<T>::buildSharedVerticesQuads(
    VolumeDataType const iso,
    std::vector<Vector3f>& vertices,
    std::vector<Quad>& quads
) {
