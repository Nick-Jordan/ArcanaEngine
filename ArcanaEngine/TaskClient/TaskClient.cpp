...
int main()
{
	...
	scheduler.schedule(new TerrainTask());
	scheduler.schedule(new TerrainTask());
	scheduler.schedule(new TerrainTask());
	scheduler.schedule(new TerrainTask());
...
void generate_terrain(double size, int level, int tx, int ty)
{
	...
	// group.
	terrain.SetSourceModule(finalPlanet);
	...
	// Write the image as a Windows bitmap file (*.bmp).
	bitmapWriter.SetSourceImage(destImage);
	bitmapWriter.SetDestFilename("full_tiles/terrain_surface-" + std::to_string(level) + "-" + std::to_string(tx) + "-" + std::to_string(ty) + ".png");
	bitmapWriter.WriteDestFile();
