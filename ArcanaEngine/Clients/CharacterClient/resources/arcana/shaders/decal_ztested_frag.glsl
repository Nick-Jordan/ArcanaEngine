...
vec4 normalRoughnessValue(vec2 texCoord, vec2 decalTexCoord, vec3 worldPosition, vec3 pixelNormal, vec3 pixelTangent, vec3 pixelBinormal);
...
void main()
{
	...
	vec4 normalRoughness = normalRoughnessValue(texCoord, decalTexCoord, worldPosition, pixelNormal, pixelTangent, pixelBinormal);
...
{	
	...
	if(u_Decal.albedoBlendFactor > 0.0f && alpha > 0.0f)
	{
		s.xyz = mix(s.xyz, texture(u_Decal.albedo, decalTexCoord).xyz * u_Decal.color.xyz, alpha * u_Decal.albedoBlendFactor);
	}
	
	if(u_Decal.specularBlendFactor > 0.0f && alpha > 0.0f)
	{
		s.w = mix(s.w, texture(u_Decal.specular, decalTexCoord).w, alpha * u_Decal.specularBlendFactor);
	}
...
{	
	...
	if(u_Decal.emissiveBlendFactor > 0.0f && alpha > 0.0f)
	{
		s.xyz = mix(s.xyz, texture(u_Decal.emissive, decalTexCoord).xyz * u_Decal.color.xyz, alpha * u_Decal.emissiveBlendFactor);
	}
	
	if(u_Decal.metallicBlendFactor > 0.0f && alpha > 0.0f)
	{
		s.w = mix(s.w, texture(u_Decal.metallic, decalTexCoord).w, alpha * u_Decal.metallicBlendFactor);
	}
...
vec4 normalRoughnessValue(vec2 texCoord, vec2 decalTexCoord, vec3 worldPosition, vec3 pixelNormal, vec3 pixelTangent, vec3 pixelBinormal)
{
	...
	if(u_Decal.normalsBlendFactor > 0.0f)
	{
		...
		s.xyz = mix(s.xyz, normal, u_Decal.normalsBlendFactor);
	}
	
	if(u_Decal.roughnessBlendFactor > 0.0f)
	{
		s.w = mix(s.w, texture(u_Decal.roughness, decalTexCoord).w, u_Decal.roughnessBlendFactor);
	}
